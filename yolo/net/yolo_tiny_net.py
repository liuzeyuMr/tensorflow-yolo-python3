...
class YoloTinyNet(Net): #定义一个类，是Net的子类

  def __init__(self, common_params, net_params, test=False):
    """
     {'image_size': 448, 'num_classes': 20,
                'batch_size':1}
net_params = {'cell_size': 7, 'boxes_per_cell':2, 'weight_decay': 0.0005} #网络结构参数
    common params: a params dict
    net_params   : a params dict
    """
    super(YoloTinyNet, self).__init__(common_params, net_params) #调用父类init函数
    #process params
    ...
    self.batch_size = int(common_params['batch_size']) #1
    self.weight_decay = float(net_params['weight_decay']) #0.0005

    if not test: #判断是否为test阶段
  ...
  def inference(self, images):
    ...
    temp_conv = self.conv2d('conv' + str(conv_num), images, [3, 3, 3, 16], stride=1) #[1,448,448,16]
    # print(temp_conv.shape)
    conv_num += 1

    temp_pool = self.max_pool(temp_conv, [2, 2], 2) #经过自定义函数池化层

    temp_conv = self.conv2d('conv' + str(conv_num), temp_pool, [3, 3, 16, 32], stride=1) #[1,224,224,32]
    # print(temp_conv.shape)
    conv_num += 1

    temp_pool = self.max_pool(temp_conv, [2, 2], 2)

    temp_conv = self.conv2d('conv' + str(conv_num), temp_pool, [3, 3, 32, 64], stride=1) #[1,112,112,64]
    conv_num += 1
    # print(temp_conv.shape)
    temp_conv = self.max_pool(temp_conv, [2, 2], 2)

    temp_conv = self.conv2d('conv' + str(conv_num), temp_conv, [3, 3, 64, 128], stride=1)#[1,56,56,128]
    conv_num += 1
    # print(temp_conv.shape)
    temp_conv = self.max_pool(temp_conv, [2, 2], 2)

    temp_conv = self.conv2d('conv' + str(conv_num), temp_conv, [3, 3, 128, 256], stride=1) #[1,28,28,256]
    conv_num += 1
    # print(temp_conv.shape)
    temp_conv = self.max_pool(temp_conv, [2, 2], 2)

    temp_conv = self.conv2d('conv' + str(conv_num), temp_conv, [3, 3, 256, 512], stride=1) #[1,14,14,512]
    conv_num += 1
    # print(temp_conv.shape)
    temp_conv = self.max_pool(temp_conv, [2, 2], 2)

    temp_conv = self.conv2d('conv' + str(conv_num), temp_conv, [3, 3, 512, 1024], stride=1) #[1,7,7,1024]
    conv_num += 1
    # print(temp_conv.shape)
    temp_conv = self.conv2d('conv' + str(conv_num), temp_conv, [3, 3, 1024, 1024], stride=1) #[1,7,7,1024]
    # print(temp_conv.shape)
    conv_num += 1 
    ...
    temp_conv = tf.transpose(temp_conv, (0, 3, 1, 2)) #(1, 1024, 7, 7)
    # print(temp_conv.shape)
    #Fully connected layer
    local1 = self.local('local1', temp_conv, self.cell_size * self.cell_size * 1024, 256)#(1,256)
    # print(local1.shape)
    local2 = self.local('local2', local1, 256, 4096)#(1,4096)
 
    local3 = self.local('local3', local2, 4096, self.cell_size * self.cell_size * (self.num_classes + self.boxes_per_cell * 5), leaky=False, pretrain=False, train=True) #输出 7*7*（20+2*5）

    n1 = self.cell_size * self.cell_size * self.num_classes #7*7*20

    n2 = self.cell_size * self.cell_size * (self.num_classes + self.boxes_per_cell )

    class_probs = tf.reshape(local3[:, 0:n1], (-1, self.cell_size, self.cell_size, self.num_classes))#[7,7,20] 获得概率值
    scales = tf.reshape(local3[:, n1:n2], (-1, self.cell_size, self.cell_size, self.boxes_per_cell)) #[7,7,2]  获得置信度
    boxes = tf.reshape(local3[:, n2:], (-1, self.cell_size, self.cell_size, self.boxes_per_cell * 4)) #[7,7,8] 表示两个框的参数

    local3 = tf.concat([class_probs, scales, boxes], 3) #最终输出的张量
