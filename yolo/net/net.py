...
class Net(object):
  ...
  def _variable_on_gpu(self, name, shape, initializer, pretrain=True, train=True):
  ...
  def conv2d(self, scope, input, kernel_size, stride=1, pretrain=True, train=True): #自定义卷积函数
    """convolutional layer
    ...
    with tf.variable_scope(scope) as scope:
      ...
      biases = self._variable_on_gpu('biases', kernel_size[3:], tf.constant_initializer(0.0), pretrain, train)
  ...
  def local(self, scope, input, in_dimension, out_dimension, leaky=True, pretrain=True, train=True):
    ...
    """
    with tf.variable_scope(scope) as scope:
      reshape = tf.reshape(input, [tf.shape(input)[0], -1]) #reshape

      weights = self._variable_with_weight_decay('weights', shape=[in_dimension, out_dimension],
                          stddev=0.04, wd=self.weight_decay, pretrain=pretrain, train=train) #获得W参数，
      biases = self._variable_on_gpu('biases', [out_dimension], tf.constant_initializer(0.0), pretrain, train)
