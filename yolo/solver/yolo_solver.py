...
class YoloSolver(Solver):
  ...
  def __init__(self, dataset, net, common_params, solver_params):
    #process params
    self.moment = float(solver_params['moment'])   #0.9
    self.learning_rate = float(solver_params['learning_rate']) #0.00001
  ...
  def solve(self):
    ...
    for step in range(self.max_iterators):
      ...
      if step % 5000 == 0:
        saver2.save(sess, self.train_dir + '/model.ckpt', global_step=step) #5000步保留一次模型
    sess.close()
